...
@Plugin(name = "PoloAppender", category = "Core", elementType = "appender")
class LoggingAgent(
    name: String,
    filter: Filter?,
    layout: org.apache.logging.log4j.core.Layout<out java.io.Serializable>
) : AbstractAppender(name, filter, layout, false) {

    override fun append(event: LogEvent) {
        val formatted = layout.toSerializable(event).toString()

        // your custom event system
        Agent.eventProvider().call(PolocloudLogEvent(formatted))

        // local runtime terminal
        if (Agent.runtime is LocalRuntime && !shutdownProcess()) {
            Agent.runtime.terminal.display(formatted)
        } else {
            print(formatted)
        }
    }
    ...
    companion object {
        ...
        fun create(
            ...
            @PluginElement("Layout") layout: org.apache.logging.log4j.core.Layout<out Serializable>?
        ): Appender {
